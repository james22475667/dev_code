...
def update_metrics():
    ...
    for host, job, extra_labels in log_data:
        key = (host, job, tuple(extra_labels.items()))  # **用 tuple 來確保 key 可以被當作字典鍵值**
    ...
    for (host, job, extra_labels_tuple), count in counts.items():
        ...
        # **確保 `labels_dict` 的 `keys` 順序與 `labels_list` 一致**
        sorted_labels_dict = {key: labels_dict[key] for key in labels_list}

        print(f"[DEBUG] 設定 `metrics` => {sorted_labels_dict} : {count}")  # 🔍 Debug
        log_host_job_count.labels(**sorted_labels_dict).set(count)
