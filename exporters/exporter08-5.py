...
log_host_job_count{container_name="bbbb",host="host_1",job_name="job_A",module_name="",service_name="aaa"} 1.0
log_host_job_count{container_name="",host="host_1",job_name="job_A",module_name="",service_name=""} 1.0
log_host_job_count{container_name="",host="host_1",job_name="job_B",module_name="",service_name=""} 1.0
log_host_job_count{container_name="",host="host_2",job_name="job_A",module_name="",service_name=""} 1.0
log_host_job_count{container_name="",host="host_2",job_name="job_C",module_name="",service_name=""} 1.0
log_host_job_count{container_name="",host="host_3",job_name="job_B",module_name="cbbb",service_name=""} 1.0
log_host_job_count{container_name="",host="host_3",job_name="job_B",module_name="",service_name=""} 2.0
...
def update_metrics():
    ...
    # **更新 Prometheus 指標**
    labels_list = sorted(list(dynamic_labels))  # 確保 labels 順序一致
    ...
    for (host, job, extra_labels), count in counts.items():
        # **確保 `labels_dict` 包含所有 `labels`**
        labels_dict = {"host": host, "job_name": job, **dict(extra_labels)}
        for label in labels_list:
            labels_dict[label] = labels_dict.get(label, "")
