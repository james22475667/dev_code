...
class LogExporter:
    ...
    def collect(self):
        ...
        with self.cache_lock:
            ...
            for key, count in self.metric_cache.items():
                ...
                metric.add_metric([labels_dict[label] for label in self.labels_list], count)  # 依 label 順序填值
    ...
    def _count_host_job(self, tmp_log_file):
        ...
        counts = {}
        dynamic_labels = {"host", "job_name"}  # 動態收集 Labels
        ...
        try:
            with open(tmp_log_file, 'r') as f:
                ...
                for row in reader:
                    ...
                    extra_labels = {k: v for k, v in row.items() if k not in ["host", "job_name"] and v}
