...
def parse_csv():
    ...
    # **確保 `dynamic_labels` 沒有錯誤標籤**
    dynamic_labels = {label.strip() for label in dynamic_labels if label and not label.startswith("{")}
...
logging.info(f"設定 Prometheus 指標，Labels: {labels_list}")  # 🔍 Debug
...
def update_metrics():
    ...
    for key, count in counts.items():
        ...
        logging.info(f"[DEBUG] 設定 `metrics` => {labels_dict} : {count}")  # 🔍 Debug
