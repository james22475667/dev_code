...
def _count_host_job(self, tmp_log_file):
    ...
    counts = {}
    dynamic_labels = {"host", "job_name"}  # 確保 labels 只包含 key，而非值

    try:
        with open(tmp_log_file, 'r') as f:
            reader = csv.DictReader(f)  # 使用 DictReader 解析 CSV
            for row in reader:
                host, job_name = row.get("host"), row.get("job_name")

                # 過濾掉 host, job_name 之外的所有 label
                extra_labels = {k.strip(): v.strip() for k, v in row.items() if k and v and k not in ["host", "job_name"]}

                # Debug: 檢查 `extra_labels` 是否正確
                logging.info(f"Extracted extra_labels: {extra_labels}")

                # 確保只加入 `keys()`，不會將值當成 key
    ...
    # Debug: 檢查 `dynamic_labels` 是否包含錯誤的 key
